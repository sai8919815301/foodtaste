...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(`Account created! UID: ${user.uid}. Redirecting...`);
      ...
      <main className="relative z-10 flex-grow flex items-center justify-center px-margin-mobile md:px-margin-desktop py-32">
        <div className="max-w-md w-full glass-panel rounded-xl p-10 ambient-glow bg-surface-container/80 backdrop-blur-md">
          ...
          <div className="text-center mb-12">
            <h2 className="text-headline-sm font-headline-sm text-primary mb-2">Join the Inner Circle</h2>
            <p className="text-body-md text-on-surface-variant italic">Create your horological profile.</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <form className="space-y-8" onSubmit={handleSubmit}>
              <div className="space-y-1">
                <label className="text-label-sm text-outline uppercase tracking-tighter" htmlFor="name">Full Name</label>
                <input 
              ...
              <div className="space-y-1">
                <label className="text-label-sm text-outline uppercase tracking-tighter" htmlFor="email">Email Address</label>
                <input 
                  className="w-full bg-transparent py-3 border-b border-outline-variant text-body-md placeholder:text-outline-variant focus:outline-none focus:border-primary transition-all" 
                  id="email" 
                  placeholder="forest@silvanus.co" 
                  type="email"
                  value={email}
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-primary text-on-primary py-5 px-8 rounded-full text-label-md hover:tracking-widest transition-all duration-500 ambient-glow active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="mt-12 pt-8 border-t border-outline-variant/30 text-center space-y-4">
              <p className="text-body-md text-on-surface-variant">Already have an account?</p>
              <Link href="/auth/login" className="block w-full border border-primary text-primary py-4 px-8 rounded-full text-label-md hover:bg-primary hover:text-on-primary hover:tracking-widest transition-all duration-500">
                SIGN IN
              </Link>
            </div>
          </div>
